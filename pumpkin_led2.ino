...
char notes[] = { 'c', 'd', 'e', 'x', 'f', 'g', 'a', 'b', 'C' };
int beats[] = { 1915, 1700, 1519, 1450, 1432, 1275, 1136, 1014, 956 };
int tempo = 300;
...
*/
int PIR_value = 0;
bool motion_mode_on = false;
void analogFlickering()
{
  int value, i;
  /*for(value = 255; value = 100; value-=10)
  {
    analogWrite(LED_PIN0, value);
    delay(30);
  }*/
//  int i;
  for(i = 0; i< 50; ++i)
  {
    analogWrite(LED_PIN0, 255);
    delay(30);
    analogWrite(LED_PIN0, 120);
    delay(30);
  }
}
...
void loop() 
{
  ...
  if(!motion_mode_on)
  {
    prev_value = PIR_value;
    PIR_value = digitalRead(PIN_PIR_SENSOR);
  }
  if(PIR_value == 1)
  {
    Serial.println("high\n");
